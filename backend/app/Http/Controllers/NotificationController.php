...
class NotificationController extends Controller
{
    public function index()
    {
        ...
        $notificationsStd = DB::select("SELECT notifications.id, users.username, users.email, users.profile_picture, notifications.message, DATE_FORMAT(notifications.created_at, '%d/%m/%Y %H:%i') as created_at FROM `notifications` JOIN users on users.id = notifications.source_id WHERE notifications.target_id = ? ORDER BY notifications.created_at DESC LIMIT 3",[$user_id]);
    ...
    {
        $user_id = Auth::user()->id;
        $notificationsStd = DB::select("SELECT notifications.id, users.username, users.email, users.profile_picture, notifications.message, DATE_FORMAT(notifications.created_at, '%d/%m/%Y %H:%i') as created_at FROM `notifications` JOIN users on users.id = notifications.source_id WHERE notifications.target_id = ? AND notifications.id<? ORDER BY notifications.created_at DESC LIMIT 3",[$user_id, $id]);
        $notifications = json_decode(json_encode($notificationsStd), true);
        for ($i=0; $i < count($notifications); $i++) { 
            $profile_picture = $notifications[$i]["profile_picture"];
            $email = $notifications[$i]["email"];
